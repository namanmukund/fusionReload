Template.test_pdp_page.onCreated(function() {
...
Template.test_pdp_breadcrumb.helpers({
...
Template.test_sticky_header.events({
...
Template.test_pdp.rendered = function() {
  Session.setTemp('onPDPpage', 1);
  Session.setTemp('onTestPDPpage', 1);
  $(window).scrollTop(0);
  $('#mask1').show();
  //set in course.js
  var pckg_id = Session.get('pckg_id');
  ...
  if (pckg_id) {
    Meteor.call('mainPdpPage', parseInt(pckg_id), userId, function(error, result) {
      Session.set('pdp_page', result);
      ...
      Meteor.call('suggestedCoursesTocListingWeb', userId, token, parseInt(grade_id), subjectId, isTestSeries, parseInt(pckg_id), function(error, result) {
...
Template.test_pdp_head.helpers({
  ...
  'randomJoinedUsers': function() {
    ...
    console.log("randomJoinedUsers", randomJoinedUsers);
...
Template.test_pdp_publisher.helpers({
...
Template.test_pdp_assets.helpers({
...
Template.test_pdp_hardware.helpers({
...
Template.test_free_contents.helpers({
...
Template.test_free_contents.events({
...
Template.test_unlock_popup.events({
  ...
  'click .buynowbtn': function(event) {
    event.preventDefault();
        ...
        console.log('express_checkout_data_before',Session.get('express_checkout_data'));
        if (userId) {
...
Template.test_pdp_hardware.events({
...
Template.test_hardware.helpers({
